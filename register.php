...
if (isset($_POST['submit'])) {
  ...
  $select_users = mysqli_query($conn, "SELECT * FROM users_info WHERE email = '$email'") or die('query failed');
  ...
  if (mysqli_num_rows($select_users) != 0) {
    ...
    if ($password != $cpassword) {
      ...
      mysqli_query($conn, "INSERT INTO users_info(`name`, `surname`, `email`, `password`) VALUES('$name','$Sname','$email','$password')") or die('Query failed');
